...
HW_DATA_PATH = "Data/hardware_data/hw_data_300_samples.csv"
...
def main():
    ...
    fixed_point_data = hw_data.applymap(lambda y: int_to_fixed_point(y, FRACT_WIDTH))
    ...
    for col in ['q_w', 'q_x', 'q_y', 'q_z']:
        plt.plot(ref_data[col][:len(hw_data)], linestyle='dotted', color=colors[col], label=f'Reference {col}')
        plt.plot(hw_data[col + '_fixed'], linestyle='solid', color=colors[col], label=f'Hardware {col}')

    plt.title('Comparison of q_w, q_x, q_y, q_z between Hardware and Reference Data')
    plt.xlabel('Sample')
    plt.ylabel('Value')
    plt.legend()
    plt.grid(True)
